...
def download_order(driver, start_time_epoch, end_time_epoch, wait_time):
    # go to order page
    url = f"https://seller-id.tokopedia.com/order"
    driver.get(url)
    time.sleep(15)
    ...
    url = f"https://seller-id.tokopedia.com/order?selected_sort=6&tab=all&time_order_created[]={start_time_epoch}&time_order_created[]={end_time_epoch}"
    driver.get(url)
    print(url)
    time.sleep(15)

    # click export orders button
    export_orders_button = driver.find_element(by=By.CSS_SELECTOR, value="div.theme-arco-space:nth-child(2) > div:nth-child(2) > button:nth-child(1)")
    export_orders_button.click()
    time.sleep(5)

    # click export button
    export_button = driver.find_element(by=By.CSS_SELECTOR, value="div.justify-end:nth-child(3) > button:nth-child(1)")
    export_button.click()
    time.sleep(wait_time)

    # click download button
    download_button = driver.find_element(by=By.CSS_SELECTOR, value="div.p-12:nth-child(2) > button:nth-child(2)")
...
def main(driver, start_date, end_date, store_name):
    ...
    for config in filtered_config:
        ...
        start_time_epoch = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S").strftime('%s') + "000"
        end_time_epoch = datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S").strftime('%s') + "000"
