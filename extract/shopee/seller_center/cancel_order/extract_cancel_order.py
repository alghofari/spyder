...
def request_file(start_time, end_time, official_store_name):
    """
    Request content from shopee cancel_order by API
    Args:
    ...
    spc_cds = [{'value': f"{cookie['name']}={cookie['value']}"}
               for cookie in cookies if cookie['name'] == 'SPC_CDS']
    cookie = '; '.join(
        [f"{cookie['name']}={cookie['value']}" for cookie in cookies])
    url = f"https://seller.shopee.co.id/api/v3/order/request_order_report?SPC_CDS={spc_cds[0]['value']}&SPC_CDS_VER=2&list_type=cancelled&start_date={start_time}&end_date={end_time}&language=id&screening_condition=order_creation_date&parcel_level_filter=0"
    ...
    headers = {
        ...
        'cookie': cookie,
        'referer': 'https://seller.shopee.co.id/portal/sale/order?type=cancelled',
        'sec-fe-ver': '21.25995',
...
def download_file_to_gcs(start_time, end_time, file_id, official_store_name, storage_client):
    ...
    headers = {
        ...
        'accept-language': 'en-US,en;q=0.9',
        'cookie': cookie,
        'referer': f'https://seller.shopee.co.id/portal/sale/order?type=cancelled',
    ...
    # Write excel to gcs
    blob = client.blob(
        f'assets/excel/shopee/cancel_order/{official_store_name}/{date.today()}/Shopee_Cancel_Order_{start_time}_{end_time}.xlsx')
...
def main(start_date, end_date, official_store_name, storage_client):
    ...
    download_file_to_gcs(start_date, end_date, file_id,
                         official_store_name, storage_client)
