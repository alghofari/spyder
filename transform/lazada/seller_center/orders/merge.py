...
def generate_query(schema_path, key_column, order_by_column, bq_project, dataset, dataset_append, table_temp, main_table):
    # --- Column name ---
    ...
    key_column_string, order_by_string = "", ""
    ...
    primary_keys_merge = [f"COALESCE(CAST(T.{key} AS STRING), 'NULL') = COALESCE(CAST(S.{key} AS STRING), 'NULL')" for key in key_column]
    ...
    print(f"merging is done for table {main_table}")