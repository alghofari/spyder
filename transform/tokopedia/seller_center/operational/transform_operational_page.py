...
from transform.tokopedia.seller_center.operational.dataframe_processing import extract_data_operational_into_dataframe
...
def main(config, gcs_logger=None):
    ...
    base_config_path = f"./transform/tokopedia/seller_center/operational/resources/"

    config_json = base_config_path + f"config_operational.json"
    ...
    prefix_scrape_directory = f"assets/json/tokopedia/operational/{official_store}/{get_today()}/"
    ...
    if detection_scrape_bucket:
        ...
        gcs_logger.log(
            f"File operational {detect_filename} is detected, process clean and upload file will be continue")
        
        # Get the dataframe
        extract_data_operational_into_dataframe(
        ...
        gcs_logger.log(
            f"File operational is not detected, please check the extract process")