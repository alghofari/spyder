...
def extract(fs, official_store_name, start_date, end_date):
    ...
    with fs.open(f'gs://sirclo-data-marketplace/assets/excel/shopee/return_order/{official_store_name}/{date.today()}/Shopee_Return_Order_{start_date}_{end_date}.xlsx') as f:
...
def load_to_bq(df, bq_client, schema):
    ...
    job = bq_client.load_table_from_dataframe(
        df, 'sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_return_order_changes', job_config = bigquery.LoadJobConfig(
...
def merge(bq_client, schema):
    ...
    query = f"""
        MERGE sirclo-prod.bronze_seller_center_commerce.shopee_return_order T
            USING sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_return_order_changes S
            ON COALESCE(CAST(T.no_pengembalian as string), "NULL") = COALESCE(CAST(S.no_pengembalian as string), "NULL")
            AND COALESCE(CAST(T.no_pesanan as string), "NULL") = COALESCE(CAST(S.no_pesanan as string), "NULL")
            AND COALESCE(CAST(T.tanggal_pesanan_dibuat as string), "NULL") = COALESCE(CAST(S.tanggal_pesanan_dibuat as string), "NULL")
...
def main(start_date, end_date, os_key):
    ...
    schema = open(os.getcwd() + '/transform/shopee/seller_center/return_order/assets/schema.json')
