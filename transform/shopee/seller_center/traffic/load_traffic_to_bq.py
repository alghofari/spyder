...
def load_from_gcs(fs, official_store_name, start_date, end_date):
    blob = fs.blob(f'assets/excel/shopee/traffic/{official_store_name}/{date.today()}/Shopee_Traffic_{end_date}_{start_date}.xlsx')
    data_bytes = blob.download_as_bytes()

    df = pd.read_excel(data_bytes, skiprows=[0,1,2])
    return df
...
def load_to_bq(bq_client, df, schema):
    ...
    job = bq_client.load_table_from_dataframe(
        df, 'sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_traffic_changes', job_config = bigquery.LoadJobConfig(
...
def merge(columns, bq_client):
    ...
    job = bq_client.query(f"""
        MERGE `sirclo-prod.bronze_seller_center_commerce.shopee_traffic` T
            USING `sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_traffic_changes` S
...
def main(os_key):
    ...
    schema = open(os.getcwd() + '/transform/shopee/seller_center/traffic/schema.json')
