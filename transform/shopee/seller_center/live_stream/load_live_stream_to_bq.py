...
def extract(fs, official_store_name, start_date, end_date):
    ...
    with fs.open(f'gs://sirclo-data-marketplace/assets/excel/shopee/live_stream/{official_store_name}/{date.today()}/Shopee_Live_Stream_{end_date}_{start_date}.xlsx') as f:
        df = pd.read_excel(f, thousands=".", decimal=",", sheet_name="Daftar Streaming")
    return df
...
def load_to_bq(df, bq_client, schema):
    ...
    job = bq_client.load_table_from_dataframe(
        df, 'sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_livestreaming_changes', job_config=bigquery.LoadJobConfig(
...
def merge(bq_client, schema):
    ...
    query = f"""
        MERGE sirclo-prod.bronze_seller_center_commerce.shopee_livestreaming T
            USING sirclo-prod.temp_append.bronze_seller_center_commerce_shopee_livestreaming_changes S
...
def main(os_key):
    ...
    schema = open(os.getcwd() + '/transform/shopee/seller_center/live_stream/schema.json')
